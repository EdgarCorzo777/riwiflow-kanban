// ============================================================
// RiwiFlow — app.js
// Works with the original login.html and board.html
...
function showToast(msg, type = "success") {
  ...
  toast.className = `rw-toast fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl font-body-sm text-body-sm shadow-lg border border-outline-variant transition-all duration-300 opacity-0 translate-y-2 ${colorClass}`;
...
function initLoginPage() {
  ...
  // Inject error element after the password field group
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      errorEl.textContent =
        "Could not connect to server. Is json-server running on port 3000?";
      errorEl.classList.remove("hidden");
      submitBtn.disabled = false;
      submitBtn.innerHTML = `Login <span class="material-symbols-outlined text-[18px]">arrow_forward</span>`;
    }
...
async function initDashboardPage() {
  ...
  try {
    ...
    showToast("Could not connect to json-server on port 3000.", "error");
    return;
  }
...
function setupSidebar() {
  ...
  // Insert user chip + logout above the "New Project" button
...
// ─── "New Project" button → open create modal (admin only) ───
function setupNewProjectButton() {
  const btn = document.querySelector("aside .px-4.mt-auto button");
  if (!btn) return;
  if (!isAdmin()) {
    btn.style.display = "none";
    return;
  }
  btn.addEventListener("click", () => openModal(null, "todo"));
}

// ─── Search ──────────────────────────────────────────────────
function setupSearch() {
  const input = document.querySelector("input[placeholder='Search tasks or files...']");
  if (!input) return;
  let debounce;
...
function renderColumn(colEl, status, filter = "") {
  ...
  // Update count badge (first span.rounded-full in header)
  const countBadge = colEl.querySelector(".flex.items-center.gap-2 span.rounded-full");
  if (countBadge) countBadge.textContent = tasks.length;

  // Replace card container contents
  const cardContainer = colEl.querySelector(".flex-1");
  if (!cardContainer) return;
  cardContainer.innerHTML = "";

  if (tasks.length === 0) {
    cardContainer.innerHTML = `
      <p class="font-body-sm text-body-sm text-outline text-center py-8 opacity-60">No tasks here</p>`;
  } else {
    tasks.forEach((task) => cardContainer.appendChild(buildCard(task)));
  }

  setupColumnAddButton(colEl, status);
}

// ─── Build task card (matches original card HTML style) ──────
function buildCard(task) {
  ...
  card.className = `task-card bg-surface ${isInProgress ? "border-l-4 border-l-primary " : ""}border border-outline-variant rounded-xl p-md shadow-sm${isDone ? " opacity-80" : ""}`;
  ...
  if (canEdit) {
    card.querySelector(".rw-edit-btn").addEventListener("click", () => {
      openModal(task.id);
    });
  }
...
function setupColumnAddButton(colEl, status) {
  ...
  addBtn.className =
    "rw-col-add material-symbols-outlined text-outline hover:text-primary transition-colors";
...
function injectModal() {
  ...
  overlay.className =
    "fixed inset-0 z-50 hidden items-center justify-center bg-on-background/50 backdrop-blur-sm p-4";
  ...
  overlay.innerHTML = `
    <div id="rwModal"
      ...
      <div class="flex items-center justify-between mb-lg">
        ...
        <button id="rwModalClose"
          class="material-symbols-outlined text-outline hover:text-on-surface transition-colors">close</button>
      </div>
      <form id="rwTaskForm" class="space-y-md">
        <div id="rwFieldTitle">
        ...
        <div class="flex gap-sm justify-end pt-sm">
          ...
          <button type="submit" id="rwSaveBtn"
  ...
  document.getElementById("rwTaskForm").addEventListener("submit", handleSave);
}
...
async function handleSave(e) {
  ...
  try {
    ...
    showToast("Error saving task. Check json-server.", "error");
  } finally {
    btn.disabled = false;
...
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("loginForm")) {
    initLoginPage();
  } else if (document.querySelector(".kanban-column")) {
    initDashboardPage();
  }
});